...
5. **Model Selection**: The Student GUI only offers response models that are runnable in the current environment. Runnable options can include canonical Salma, Fatma, and Khadija fine-tuned models, base Qwen, Groq-hosted chat, OpenAI-hosted chat, or deterministic fallback depending on installed packages, configured keys, and local base-model cache/download availability. Backend Tracking lists every saved adapter/experiment.
