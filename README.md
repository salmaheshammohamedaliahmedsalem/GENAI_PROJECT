...
5. **Model Selection**: The Student GUI shows one canonical Salma fine-tuned model and one canonical Fatma fine-tuned model, plus base Qwen, Groq-hosted chat, OpenAI-hosted chat, or deterministic fallback depending on installed packages and configured keys. Backend Tracking lists every saved adapter/experiment.
